...
class ToolRegistry:
    ...
    def _load_dotenv() -> None:
        """Load .env into os.environ if present, so tools can find API keys.

        Delegates to ``tools.base_tool.load_project_dotenv`` on purpose: both
        entry points must apply the same allow-list, and a second copy of the
        parser is how the unfiltered version ended up in two places. See
        ``lib/env_allowlist`` for why an unfiltered copy is dangerous.
        """
        from tools.base_tool import load_project_dotenv
